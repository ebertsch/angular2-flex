...
import {sassBuildTask, tsBuildTask, execNodeTask, execNpmScript, copyTask, sequenceTask} from '../task_helpers';
...
/** Path to the root of the Angular2 Flex component library. */
...
/** Builds the UMD bundle for all of Angular2 Flex. */
task(':build:components:rollup', [':build:components:inline'], () => {
  ...
  // Rollup the angular2-flex UMD bundle from all ES5 + imports JavaScript files built.
  return rollup({
    ...
    const result = bundle.generate({
      moduleName: 'angular2flex',
      format: 'umd',
      globals,
      sourceMap: true,
      dest: path.join(DIST_COMPONENTS_ROOT, 'angular2-flex.umd.js')
    });
    ...
    // dist/angular2-flex, we need to kill a few `../`.
    result.map.sources = result.map.sources.map((s: string) => s.replace(/^(\.\.\/)+/, ''));

    writeFileSync(path.join(DIST_COMPONENTS_ROOT, 'angular2-flex.umd.js'), result.code, 'utf8');
    writeFileSync(path.join(DIST_COMPONENTS_ROOT, 'angular2-flex.umd.js.map'), result.map, 'utf8');
  });
});
...
// ));
task(':build:components:ngc', ['build:components'], execNpmScript('compile'));
